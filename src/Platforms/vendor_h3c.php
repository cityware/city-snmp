...
if (substr($sysDescr, 0, 20) == 'H3C Comware software') {
    ...
    preg_match('/H3C Comware software. (.+) Product Version\s(.+)\. Copyright/', $sysDescr, $matches);
    ...
    preg_match('/H3C Switch (.+) Software Version (.+)Copyright/', $sysDescr, $matches);
    ...
    $sysDescr = trim(preg_replace('/\s+/', ' ', $sysDescr));
    preg_match('/H3C Comware Platform Software, Software Version (.+) H3C (.+)Copyright/', $sysDescr, $matches);
