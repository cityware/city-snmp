...
if (substr($sysDescr, 0, 14) == 'Alcatel-Lucent') {
    ...
    if (substr($sysDescr, 0, 18) == 'Alcatel-Lucent OS9') {
        preg_match('/Alcatel-Lucent (OS.+CFM) ([0-9A-Za-z\(\)\.]+) GA,\s([a-zA-Z]+)\s(\d+),\s(\d+)\./', $sysDescr, $matches);
        ...
        preg_match('/Alcatel-Lucent (OS.+) ([0-9A-Za-z\(\)\.]+) GA,\s([a-zA-Z]+)\s(\d+),\s(\d+)\./', $sysDescr, $matches);
        ...
        if (!empty($model)) {
            ...
            preg_match('/Alcatel-Lucent ([0-9A-Za-z\(\)\.]+) GA,\s([a-zA-Z]+)\s(\d+),\s(\d+)\./', $sysDescr, $matches);
